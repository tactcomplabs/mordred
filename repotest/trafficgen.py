    ...
    for y in range(y_size):
        for x in range(x_size):
            ...
            for k in range(local_ports):
                ...
                lcl_ep = sst.Component(ep_name, "merlin.trafficgen")
                lcl_ep.addParams({
                    "id" : ep_num,
                    "num_peers" : num_eps,
                })
                lcl_ep.addParams(TrafficGenParams)
                lcl_ep.addParam("PacketDest.RangeMax", num_eps)
                lcl_ep.addParams(TrafficGenMessagingParams)
                lcl_ep_iface = lcl_ep.setSubComponent("networkIF", "mordred.mordredNIC")

                lcl_ep_iface.addParams(MordredNicParams)
    ...
    def gen_endpoints(self):
        ...
        for i in range(self.num_routers):
            for j in range(self.k):
                ...
                ep_num = i*self.k + j
                ep = sst.Component(ep_name, "merlin.trafficgen")
                ep.addParams({
                    "id" : ep_num,
                ...
                ep.addParam("PacketDest.RangeMax", self.num_endpoints)
                endpoints.append(ep)
