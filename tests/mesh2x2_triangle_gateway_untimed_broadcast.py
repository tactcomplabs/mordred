# mesh2x2_triangle_gateway_untimed_broadcast.py
#
...
#
# Variant of mesh2x2_triangle_gateway_ucie_testnic.py with
# send_untimed_broadcast=true.
#
# This is the test that exercises GatewayTopology's domain-wide broadcast
# dedup: mesh C here can be entered by a given broadcast via TWO different
# physical routers -- directly from A (meshC_rtr_0_0) and relayed via B
# (meshC_rtr_0_1) -- each running its own separate GatewayTopology C++
# instance with no other shared state. Without dedup, C would flood its own
# mesh twice for the same broadcast (once from each entry router), and B
# and C would keep re-relaying each other's copies back and forth
# indefinitely. See GatewayTopology.h's class doc comment and
# GatewayTopology.cc's sSeenBroadcastOrigins for the mechanism (a domain-
# wide, id_base-keyed "already flooded this origin" set, consulted only at
# genuine external gateway entries, never at ordinary intra-domain hops).
#
# Same completion criterion as the other untimed_broadcast tests: every
# test_nic's init handshake requires init_broadcast_count to reach EXACTLY
# num_peers-1 (a strict equality check in nic.cc). A broken dedup fails in
# BOTH directions here: too little relay and some node's count never
# reaches num_peers-1 (hangs); too much (a double-delivered duplicate) and
# the count overshoots num_peers-1 and never satisfies the strict equality
# either (also hangs). A clean "Simulation is complete" is a real,
# rigorous confirmation that every one of the 12 nodes' broadcasts reached
# all 11 others EXACTLY once, despite C's two entry points.
#
# See mesh2x2_triangle_gateway_ucie_testnic.py for the full topology; only
...
testname = "mesh2x2_triangle_gateway_untimed_broadcast"
...
FixedTestNicParams = {
    ...
    "send_untimed_broadcast" : "true",   # <-- the only real change from mesh2x2_triangle_gateway_ucie_testnic.py
}
