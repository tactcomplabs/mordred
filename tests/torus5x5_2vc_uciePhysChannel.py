...
#
# uciePhysChannel variant of torus5x5_2vc_testnic.py.
#
# Every link in the 5x5 torus uses prydwen.uciePhysChannel as the physical
# transport.  VCs (num_vcs=2) are managed above the port-control layer, so
# the UCIe channel uses a single VN (num_vns_per_stack="1").  Topology and
# traffic parameters are identical to the original.
#
...
testname = "torus5x5_2vc_uciePhysChannel"
...
MordredNicPCParams = {
    ...
    "num_vcs"         : "2",  # must match FixedRtrParams["num_vcs"] — see MordredNicPC's num_vcs param doc
}
