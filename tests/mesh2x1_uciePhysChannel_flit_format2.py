...
#
# Variant of mesh2x1_uciePhysChannel.py using UCIe FLIT format 2
# (68B wire, 64B payload) instead of the default format 5 (256B, 240B payload).
#
# All other mordred UCIe tests use flit_format=5.  Format 2 has a much
# smaller payload (64B vs 240B), which exercises a different fragmentation
# path in UCIePhysChannel: mordred's 16-bit flits are 2B each, so a 4-flit
# packet (8B) still fits in a single format-2 UCIe flit, but the
# serialization delay and credit accounting use the format-2 wire width
# (68B) and payload size (64B) instead of format-5 values.
#
# UCIe spec constraint: format 2 requires lane_speed_gts <= 32 GT/s.
# This test uses 16 GT/s (UCIe 1.0 standard speed) to satisfy that limit.
#
# Topology:
...
testname = "mesh2x1_uciePhysChannel_flit_format2"
...
UCIeParams = {
    ...
    "num_vns_per_stack" : "1",
    "credits_per_vn"    : "32",
    "flit_format"       : 2,    # 68B wire, 64B payload (vs format 5's 256B/240B)
    "num_modules"       : 1,
    "num_lanes"         : 16,
    "lane_speed_gts"    : 16,   # format 2 requires <= 32 GT/s; use 16 GT/s
    "verbose"           : 1,
}
