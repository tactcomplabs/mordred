    ...
    init_broadcast_count(0)
{
  ...
  output = new SST::Output("MordredNIC[" + getName() + ":@p:@t]: ", verbosity, 0, Output::STDOUT);
    ...
    if ( !link_control ) {
        output->fatal(CALL_INFO,1,"Error: no LinkControl object loaded into test_nic\n");
    }

    last_target = net_id;
    next_seq = new int[(uint32_t)num_peers];
  ...
  output->verbose( CALL_INFO, 5, 0, "Created testNic=%d with %d peers\n", net_id, num_peers );
...
private:
    ...
    ImplementSerializable(SST::Mordred::MyRtrEvent);
};
