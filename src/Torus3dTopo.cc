...
Torus3DTopo::Torus3DTopo( ComponentId_t id, Params& params,
  ...
  perPortConnectedRtr( connected_ports )
{
  ...
  if( ( xId >= xDim ) || ( yId >= yDim ) || ( zId >= zDim ) ) {
    output->fatal(
      CALL_INFO, -1, "xId=%" PRId32 " >= xSize=%" PRId32 " OR yId=%" PRId32 " >= ySize=%" PRId32 " OR zId=%" PRId32 " >= zSize=%" PRId32 "\n",
      xId, xDim, yId, yDim, zId, zDim
    );
  }
  ...
  output->verbose( CALL_INFO, 1, 0, "Torus3DTopo constructed; rtr_id=%" PRIu32 ", num_ports=%" PRIu32 ", local_ports=%" PRIu32 "\n",
    rtr_id, numPorts, numLocalPorts);
...
uint32_t Torus3DTopo::routePacket( uint32_t dest ) {
  ...
  if ( dest_rtr_id != rtrId ) {
    ...
    if ( dest_x != xId ) {
      ...
      if ( dz_neg < 0 ) dz_neg += (int32_t)zDim;
      int32_t dz_pos = (int32_t)dest_z - (int32_t)zId;
      if ( dz_pos < 0 ) dz_pos += (int32_t)zDim;
...
void Torus3DTopo::routeUntimedBroadcastPacket( uint32_t receive_port_id, MordredInitEvent* init_ev, std::vector<Event*>& output_events ) {
