    ...
    netID( -1 )
{
  ...
  if ( !inbufSize.hasUnits("b") && !inbufSize.hasUnits("B") )
    output->fatal( CALL_INFO, -1,
      "input_buf_size must be specified in bits or bytes: %s\n",
  ...
  if ( !outbufSize.hasUnits("b") && !outbufSize.hasUnits("B") )
    output->fatal( CALL_INFO, -1,
      "output_buf_size must be specified in bits or bytes: %s\n",
...
void MordredNicPC::init( uint32_t phase ) {
  ...
  case 1: {
    ...
    output->verbose( CALL_INFO, 5, 0,
      "Phase 1: connected to [Rtr.Port]=[%" PRIu32 ".%" PRIu32 "]\n",
      rtrId, rtrPort );
    delete init_ev;
    break;
  }
  ...
  default: {
    ...
    while ( (ev = physChannel->recvUntimedData()) != nullptr ) {
      ...
      if ( base_ev->getType() == baseMordredEvent::CREDIT ) {
        ...
        output->verbose( CALL_INFO, 5, 0,
          "Received credit vn=%" PRIu32 ", credits=%" PRId32 "\n",
        ...
        output->verbose( CALL_INFO, 5, 0,
          "Received unexpected event type=%d in init default\n",
          (int)base_ev->getType() );
        delete ev;
      }
    }
...
void MordredNicPC::complete( uint32_t phase ) {
  ...
  while ( (ev = physChannel->recvUntimedData()) != nullptr ) {
    ...
    if ( base_ev->getType() == baseMordredEvent::CREDIT ) {
      ...
      output->verbose( CALL_INFO, 5, 0,
        "Received unexpected event type=%d in complete\n", (int)base_ev->getType() );
      delete ev;
    }
  }
}
...
void MordredNicPC::finish() {
  ...
  const double avg_flits =
    totalPackets > 0 ? (double)totalNumFlits / (double)totalPackets : 0.0;
...
bool MordredNicPC::send( Request* req, int vn ) {
  ...
  if ( numVns <= u_vn )
    output->fatal( CALL_INFO, -1,
      "send: requested vn=%d is invalid (numVns=%" PRIu32 ")\n", vn, numVns );
...
SimpleNetwork::Request* MordredNicPC::recv( int vn ) {
  ...
  if ( numVns <= u_vn )
    output->fatal( CALL_INFO, -1,
      "recv: requested vn=%d is invalid\n", vn );
  if ( inBuf.at( u_vn ).empty() ) return nullptr;
  ...
  if ( req->dest != netID )
    output->fatal( CALL_INFO, -1,
      "Packet with dest=%" PRId64 " received by netID=%" PRId64 "\n",
      req->dest, netID );
  return req;
}
...
bool MordredNicPC::spaceToSend( int vn, int num_bits ) {
  ...
  if ( numVns <= u_vn )
    output->fatal( CALL_INFO, -1,
      "spaceToSend: requested vn=%d is invalid\n", vn );
...
bool MordredNicPC::requestToReceive( int vn ) {
  ...
  if ( numVns <= u_vn )
    output->fatal( CALL_INFO, -1,
      "requestToReceive: requested vn=%d is invalid\n", vn );
...
void MordredNicPC::processIncoming( SST::Event* ev ) {
  ...
  default:
    output->fatal( CALL_INFO, -1,
      "processIncoming: unknown event type=%d\n", (int)bev->getType() );
  }
}
...
MordredInitEvent* MordredNicPC::getInitEvent( MordredInitEvent::Commands cmd ) {
  ...
  if ( ev->command != cmd )
    output->fatal( CALL_INFO, -1,
      "getInitEvent: unexpected command %d, expected %d\n",
