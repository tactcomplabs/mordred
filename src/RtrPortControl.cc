...
RtrPortControl::RtrPortControl(
  ...
  output               = new Output(
    "RtrPortControl[[" + std::to_string( rtrId ) + "." + std::to_string( portId ) + "]:@p:@t]: ", verbosity, 0, Output::STDOUT
  );
  ...
  if( !flit_size_ua.hasUnits( "b" ) && !flit_size_ua.hasUnits( "B" ) ) {
    output->fatal(
      CALL_INFO,
      -1,
      "PortControl: flit_size must be specified in either "
      "bits (b) or bytes (B): %s\n",
  ...
  if( !buf_size_ua.hasUnits( "b" ) && !buf_size_ua.hasUnits( "B" ) ) {
    output->fatal(
      CALL_INFO,
      -1,
      "RtrPortControl: input_buf_size must be specified in either "
      "bits (b) or bytes (B): %s\n",
  ...
  if( flitSize > inBufSize ) {
    output->fatal(
      CALL_INFO,
      1,
      "Invalid configuration; flit_size=%" PRIu32 "b > input_buf_size=%" PRIu32 "b (buf cannot hold a flit)\n",
      flitSize,
      inBufSize
    );
  }
  ...
  if( !buf_size_ua.hasUnits( "b" ) && !buf_size_ua.hasUnits( "B" ) ) {
    output->fatal(
      CALL_INFO,
      -1,
      "RtrPortControl: outbuf_size must be specified in either "
      "bits (b) or bytes (B): %s\n",
      buf_size_ua.toStringBestSI().c_str()
    );
  }
  ...
  if( flitSize > outBufSize ) {
    output->fatal(
      CALL_INFO,
      1,
      "Invalid configuration; flit_size=%" PRIu32 "b > output_buf_size=%" PRIu32 "b (buf cannot hold a flit)\n",
      flitSize,
      outBufSize
    );
  }
  ...
  output->verbose(
    ...
    1,
    0,
    "Constructor complete; [Rtr.Port]=[%" PRIu32 ".%" PRIu32 "], inbuf_size=%" PRIu32 "b, outbuf_size=%" PRIu32 "b\n",
    rtrId,
    portId,
    inBufSize,
    outBufSize
  ...
  case 1: {
    ...
    if( connectionType == ROUTER ) {
      ...
      output->verbose(
        ...
        5,
        DEBUG_INIT_PHASE,
        "Received init packets from [Rtr.Port]=[%" PRIu32 ".%" PRIu32 "]\n",
        connectedRtrId,
        connectedPortId
      );
      delete init_ev;
    ...
    while( ( ev = link->recvUntimedData() ) != nullptr ) {
      ...
      if( base_ev->getType() == baseMordredEvent::CREDIT ) {
        ...
        output->verbose(
          ...
          5,
          DEBUG_INIT_PHASE,
          "Received initCredit event vc=%d, credits=%d; cur_credits=%d\n",
          credit_ev->vc,
          credit_ev->credits,
          outStateVec.at( credit_ev->vn ).at( credit_ev->vc ).destCredits
        );
        delete ev;
