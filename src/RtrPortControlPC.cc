...
RtrPortControlPC::RtrPortControlPC(
  ComponentId_t id, Params& params, TopologyAPI* topology, RtrOwnedSharedObjs* rtr_shared_objs, uint32_t rtr_num, uint32_t port_num
)
  : RtrPortControlAPI( id ), topo( topology ), rtrId( rtr_num ), portId( port_num ), rtrSharedObjs( rtr_shared_objs ) {
  const auto verbosity = params.find<uint32_t>( "verbose", 5 );
  output               = new Output(
    "RtrPortControlPC[[" + std::to_string( rtrId ) + "." + std::to_string( portId ) + "]:@p:@t]: ", verbosity, 0, Output::STDOUT
  );
  ...
  if( !flit_size_ua.hasUnits( "b" ) && !flit_size_ua.hasUnits( "B" ) ) {
    output->fatal(
      CALL_INFO,
      -1,
      "RtrPortControlPC: flit_size must be specified in bits (b) or bytes (B): %s\n",
  ...
  if( !buf_size_ua.hasUnits( "b" ) && !buf_size_ua.hasUnits( "B" ) )
    output->fatal(
      CALL_INFO, -1, "RtrPortControlPC: input_buf_size must be in bits (b) or bytes (B): %s\n", buf_size_ua.toStringBestSI().c_str()
    );
  ...
  if( !buf_size_ua.hasUnits( "b" ) && !buf_size_ua.hasUnits( "B" ) )
    output->fatal(
      CALL_INFO,
      -1,
      "RtrPortControlPC: output_buf_size must be in bits (b) or bytes (B): %s\n",
      buf_size_ua.toStringBestSI().c_str()
    );
  if( buf_size_ua.hasUnits( "B" ) )
    buf_size_ua *= UnitAlgebra( "8b/B" );
...
void RtrPortControlPC::init( unsigned int phase ) {
  ...
  case 1: {
    ...
    if( init_ev->command == MordredInitEvent::REPORT_ROUTER ) {
      ...
      output->fatal( CALL_INFO, -1, "RtrPortControlPC: unexpected command=%d in phase 1\n", (int) init_ev->command );
    }
    delete raw;
...
void RtrPortControlPC::ClockTick( Cycle_t /*cycle*/ ) {
  ...
  // Output: send one flit per tick using round-robin across VN/VC
  bool sent = false;
  for( uint32_t i = 0, vn = flit_vn_rr; i < numVns; i++, vn = ( vn != ( numVns - 1 ) ) ? vn + 1 : 0 ) {
    for( uint32_t j = 0, vc = flit_vc_rr; j < numVcs; j++, vc = ( vc != ( numVcs - 1 ) ) ? vc + 1 : 0 ) {
      ...
      if( !out.outBuf.empty() && out.destCredits > 0 &&
        ...
        if( ( flit->ftype == MordredFlit::HEAD ) && ( flit->getRequest()->getTraceType() == SimpleNetwork::Request::FULL ) ) {
          output->output(
...
void RtrPortControlPC::processIncoming( SST::Event* ev ) {
  ...
  case baseMordredEvent::FLIT: {
    ...
    if( ( flit->ftype == MordredFlit::HEAD ) && ( flit->getRequest()->getTraceType() != SimpleNetwork::Request::NONE ) ) {
      output->output(
...
MordredInitEvent* RtrPortControlPC::getInitEvent( MordredInitEvent::Commands cmd ) {
  ...
  if( ev->command != cmd )
    output->fatal( CALL_INFO, -1, "Unexpected init command %d, expected %d\n", (int) ev->command, (int) cmd );
  return ev;
}
...
void RtrPortControlPC::returnCredit() {
  for( uint32_t i = 0, vn = credit_ret_vn_rr; i < numVns; i++, vn = ( vn != ( numVns - 1 ) ) ? vn + 1 : 0 ) {
    for( uint32_t j = 0, vc = credit_ret_vc_rr; j < numVcs; j++, vc = ( vc != ( numVcs - 1 ) ) ? vc + 1 : 0 ) {
      ...
      if( in.retCredits != 0 ) {
        physChannel->send( new MordredCreditEvent( vn, vc, in.retCredits ), static_cast<int>( vn ) );
        in.retCredits = 0;
        break;
      }
    }
